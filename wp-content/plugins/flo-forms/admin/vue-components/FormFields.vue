...
" >
      ...
      <div v-bind:class="{ 'hidden': (schema.groups.length == 2 && !schema.groups[0].fields.length && newFormState && isProVersion == 1) }">
        ...
        <div id="fields-settings">
          <ul class="fields-controls" v-for="(group, group_id) in schema.groups" >
            ...
            <li v-for="(value, field_id) in schema.groups[group_id].fields" :data-field_id="'fid_' + value.field_id" class="field-settings">
              <div class="field-settings-label field-settings-label--label-description"  v-if="value.type != 'submit' && value.type != 'checkbox' && !value.styleClasses.includes('flo-section-break')">
                <div class="">
                  <label class="settings-label" v-if="value.type == 'label'"> Message value: </label>
                  <label class="settings-label" v-else> Field Label: </label>
                  <FloEditor v-if="value.type == 'label'" :value="value.help"  v-model="value.help"></FloEditor>
                ...
                <span class="hint" v-if="value.type == 'label'">
                  NOTE! If you need to use '<' or '>' characters, we recommend using the related
                  <a href="https://www.w3schools.com/html/html_entities.asp" target="_blank">HTML Character Entities</a> instead.
                  Because these characters may break the form sometimes.
                </span>
              </div>
              ...
              <div class="field-settings-label field-settings-label--ckbx-description"  v-if="value.type == 'checkbox'">
                <div class="">
                  ...
                  <FloEditor :value="value.help"  v-model="value.help"></FloEditor>
              ...
              <div class="field-settings-label" v-if=" typeof value.type != 'undefined' && value.type == 'pikaday' && typeof value.pikadayOptions != 'undefined' ">
                <div class="">
                  ...
                  <div class="hint">
                    If you need a custom date format, you can set or edit it from <a :href="settingsPageUrl+'&tab=settings'" target="_blank">general settings page</a>.
              ...
              <div class="field-settings-label" v-if=" typeof value.type != 'undefined' && value.type == 'pikaday' && typeof value.pikadayOptions != 'undefined' ">
                <div class="">
                  <label class="settings-label width_100"> First Day of the Week: </label>
                  <select class="width_100" v-model="value.pikadayOptions.firstDay" @change="change_first_week_day($event, field_id,group_id)">
                    <option v-for="(dayLabel,dayValue) in firstWeekDayOptions"
                            :selected="dayValue == value.pikadayOptions.firstDay ? 'selected' : ''"
                            :value="dayValue"
                    >
                      {{dayLabel}}
                    </option>
                    ...
                    <div v-for="(cl_group, group_rule_id) in value.attributes.input.conditional_logic_groups" class="cl-rules--group-container">
                      <fieldset>
                        ...
                        <div v-for="(rule_data, rule_id) in cl_group.rules" class="cl-rules--rule-container">
                          ...
                          <div class="cl-rules--rule-property" v-else-if="typeof allFields[rule_data.rule_select] != 'undefined' && typeof allFields[rule_data.rule_select].type != 'undefined' &&  allFields[rule_data.rule_select].type == 'checkbox' ">
                            <select v-model="rule_data.rule_input">
                              <option :selected="true == rule_data.rule_input ? 'selected' : ''" :value="true" >

                                Checked
                              </option>
                              <option :selected="false == rule_data.rule_input ? 'selected' : ''" :value="false" >
          ...
          <div class="form-settings--styling" v-show="formStylingActiveTab == 'button' ">
            ...
            <div class="flex jc-space-between button-styles-digit ff_margin_bottom_25">
              ...
              <div class="buttonp">
                ...
                <input type="number" v-model="formSettings.styling.button.padding_x" :name="'flo_form_settings[styling][button][padding_x]'">
              </div>
            </div>
          ...
          <div class="form-settings--styling" v-show="formStylingActiveTab == 'fonts' ">
            ...
            <div class="form-settings--styling-typography ">
              ...
              <div v-show="showCustomFontsOptions()" class="">
                ...
                <div class="hint">
                  <b><u>Note!</u></b> The font sizes set here are responsive, meaning they will adjust depending on the screen size. Also the theme styles may alter the size
                  on the front end and make it look different(smaller or larger) - In this case increase or decrease the font size according to your needs.
                </div>
                <br/><br/>
        ...
        <div id="form-settings" class="form-settings">
          ...
          <div class="form-settings--settings" v-show="formSettingsActiveTab == 'settings'">
            ...
            <div class="field-settings-label border-bottom" for="email-subject">
              ...
              <span class="tab-title hint">
                You can use any of the existing fields value by using %field_id%. For example: %fid_95908% - where fid_95908 is the ID of one of the form’s input fields. To find out the ID of a field, just click on it in the preview on the right and it will show on the left side at the very bottom before the Save button.
              </span>
            </div>
            ...
            <div class="field-settings-label field-settings-label--email-confirmation-options flo-fancy-checkbox">
              ...
              <div class="email-confirmation-container" v-show="formSettings.enable_email_confirmation == '1'">
                ...
                <span class="tab-title hint">
                  If %field_email% is used, then the confirmation email will be sent to the email entered by the user in the ‘email field’, if such field is available in the form.
                </span>
                ...
                <span class="tab-title hint">
                  Use %all_fields% as a short code if you want to show all submitted form fields. Also you can use any of the existing fields value by using %field_id%. For example: %fid_95908% - where fid_95908 is the ID of one of the form’s input fields. To find out the ID of any field, just click on it in the preview on the right and it will show on the left side at the very bottom before the Save button. Additionally, any regular text message can be used to replace or complement the submitted fields.
                </span>
              </div>
          ...
          <div class="form-settings--integrations" v-show="formSettingsActiveTab == 'integrations'">
            ...
            <div v-if="Object.keys(formIntegrations).length == 0" class="form-settings--integrations-no_integrations">
              There are no integrations available. Integrations with other services can be added via additional plugins.
            </div>
            <div v-else v-for="(integration, interg_index) in formIntegrations" :key="interg_index" :class="integration.wrapper_class" class="colapsable-option--outer-container" >
              <div class="colapsable-option " :class="'colapsable-option_'+interg_index">
                ...
                <div class="colapsable-option--wrapper">
                  <component v-bind:is="integration.custom_tag" v-bind:form-schema=JSON.stringify(schema) ></component>
                </div>
              </div>
            </div>

            <a class="form-settings--integrations-linkall" href="https://flothemes.com/floforms/" target="_blank">
        ...
        <div class="flo-hotmail-warning" v-if="formSettings['send-to-email'].includes('@hotmail.com')">
            You are using a <u>Hotmail</u> adress as the recipient email. There are known delivery issues to Hotmail. Use please another email address to avoid email delivery problems.
        </div>
    ...
    <div class="row form-preview topLabel " v-bind:class="{ 'two-columns': schema.groups.length == 3 }">
      <a v-if="isProVersion != 1" href="https://docs.flothemes.com/floforms/" target="_blank" class="forms-docs-link-icn forms-docs-link-icn--free dashicons dashicons-info" title="Documentation">
      </a>
      ...
      <div class="no-fields-msg" v-if="schema.groups.length == 2 && !schema.groups[0].fields.length && newFormState">
        ...
        <div v-bind:class="getPluginVersionClass()" class="start-how-to-tabs" v-if="this.isProVersion == 1">
          <div id="how-it-works" class="start-how-to-tabs--content">
            ...
            <div class="text">
              Watch the video below to check out what options and settings are inside the FloForm plugin. This video goes over the main areas of the plugin including free and Pro features
            </div>
            <div class="video" style="text-align: center;">
              <iframe width="560" height="315" src="https://www.youtube.com/embed/8PULnBCdFzQ" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="preview-tabs"  >
          <ul v-if="isProVersion == 1" class="preview-tabs--wrapper" >
            <li class="form-design">
              ...
              <a href="https://docs.flothemes.com/floforms/" target="_blank" class="forms-docs-link-icn dashicons dashicons-info" title="Documentation">
