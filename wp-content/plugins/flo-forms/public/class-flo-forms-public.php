...
if(!class_exists('Flo_Forms_Public')){
	class Flo_Forms_Public {
      ...
      // fn flo_get_options_field_objects()
			//if(has_shortcode( $post->post_content, 'floform' ) || $running_flo_shortcode === true ) {
				...
				wp_enqueue_script( 'flo_vue_app_js', plugin_dir_url(__FILE__).'../dist/js/app.js', array('jquery','wp-i18n'), $this->version, $in_footer = true );
				//wp_enqueue_script( 'flo-forms-public', plugin_dir_url( __FILE__ ) . 'js/flo-forms-public.js?v2', array( 'jquery' ), $this->version, true );

				//wp_set_script_translations( 'flo_vue_app_js', 'flo-forms' );
				//wp_set_script_translations( 'flo_vue_app_js', 'flo-forms', plugin_dir_path( __FILE__ ) . 'languages' );
    ...
    /*
     * Retrieve Attachment's paths
     * */
    static public function get_attachments_paths($attachmets) {

      // attachments has the following format:
      /*
      $attachmets[] = array(
        'name' => $file['name'],
      ...
      */
      ...
      $attachmets_paths = array();

      foreach ($attachmets as $attachment) {
        $attachmets_paths[] = $attachment['system_path'];
      }

      return $attachmets_paths;
    }
    ...
    static public function flo_submit_form(){
      ...
      do_action( 'flo_forms_before_send_mail', $post_data );
			...
			// get only the attachment's system paths which are required to be attached to the emails
      $attachmet_paths = self::get_attachments_paths($attachments);
      ...
      $maybe_send_confirmation_email = self::send_confirmation_email($form_options,$submited_info['table'],$submited_info['user_email'], $_POST);
			...
			if(true !== $maybe_send_email){ // if for somereasont he email was not delivered
			  if(!isset($form_options['send-to-email']) ||
